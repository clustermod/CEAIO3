...
def getDefinedStrings(filepath):
    ...
    with open(filepath, 'r', encoding="latin-1") as file:
        content = file.read()
        srch = re.compile('Key ID\=\"(STR_{0}_[_a-zA-Z0-9]*)"'.format(CONFIG["prefix"].upper()), re.IGNORECASE)
...
def getStringUsage(filepath):
    ...
    with open(filepath, 'r') as file:
        ...
        srch = re.compile('(STR_{0}_[_a-zA-Z0-9]*)'.format(CONFIG["prefix"].upper()), re.IGNORECASE)
        fileStrings = srch.findall(content)

        srch = re.compile('[^E][CL]STRING\(([_a-zA-Z0-9]*)\)', re.IGNORECASE)
        modStrings = srch.findall(content)
        for localString in modStrings:
            fileStrings.append("STR_{0}_{1}_{2}".format(CONFIG["prefix"].upper(), selfmodule, localString))

        srch = re.compile('E[CL]STRING\(([_a-zA-Z0-9]*),([_a-zA-Z0-9]*)\)')
        exStrings = srch.findall(content)
        for (exModule, exString) in exStrings:
            fileStrings.append("STR_{0}_{1}_{2}".format(CONFIG["prefix"].upper(), exModule, exString))

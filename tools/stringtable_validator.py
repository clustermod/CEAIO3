...
def check_stringtable(filepath):
    ...
    if root.get("name") != CONFIG["prefix"].upper():
        print("  ERROR: Invalid name attribute '{}' for Project tag, must be '{}'.".format(root.get("name"), CONFIG["prefix"].upper()))
        errors += 1
    ...
    if package is None:
        ...
        key_ids = []
        key_prefix = "STR_{}_{}_".format(CONFIG["prefix"].upper(), package_name)
